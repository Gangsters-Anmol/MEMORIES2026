...
export function switchTab(tabName) {
  ...
  if (tabName === 'profile') _renderProfileTab();
...
function _renderSearchCard(u) {
  ...
  const online = u.online ? '<div class="search-user-online"></div>' : '';
  const lastSeen = u.lastSeen ? `Last seen ${_relativeTime(u.lastSeen.toDate?.() || new Date(u.lastSeen))}` : (u.online ? 'Online' : 'Offline');
    ...
    <div class="search-user-card" data-uid="${u.uid}">
      ...
      <div class="search-user-info">
        <div class="search-user-name">
          ${_esc(u.displayName || u.email || 'Unknown')}
        ...
        <div class="search-user-meta">
          ${roleLabel} · ${lastSeen}
          ${u.followersCount ? `· ${u.followersCount} followers` : '· 0 followers'}
        </div>
      </div>
...
async function _renderProfileTab() {
  ...
  if (_profileLikesUnsub) { _profileLikesUnsub(); _profileLikesUnsub = null; }
}
      ...
      const cards = await Promise.all(followers.map(async uid => {
        try {
          ...
          return `<div class="search-user-card" onclick="window.location.href='profile.html?user=${uid}'">
            ...
            <div class="search-user-info">
              <div class="search-user-name">${_esc(u.displayName || u.email || 'Unknown')}</div>
